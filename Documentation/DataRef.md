...
const converter = (sequenceData) => {
  const {tasks, columns, columnOrder} = sequenceData;
  const track = []

  columnOrder.map((block, i) => {
    const currentBlock = sequenceData.columns[block]
    const tempBlock = []

    (currentBlock.length > 0) && currentBlock.taskIds.map((note, j) => {
      const currentNote = sequenceData.tasks[note]
      const tempObj = {}
...
const sequenceConversion = [ //Track
  [ //Block
    { //Note
      library: 'keys',
      name: 'Keys_1',
